...
## The border size that cancels a drag. This is required because godot does not detect a drag that leaves the touchscreen
@export var screen_border: int = 20
...
func resize():
	var scrn = get_viewport_rect()
	_allowed_rect = Rect2i(scrn.position.x + screen_border, scrn.position.y + screen_border, scrn.size.x - 2 * screen_border, scrn.size.y - 2 * screen_border)
	
	# Adjust control size
	size.x = scrn.size.x / 2
	size.y = scrn.size.y / 3
	position.x = 0
	position.y = scrn.end.y - size.y
	
	_base_default_position = Vector2(100, size.y - _base.size.y -100)
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventScreenTouch:
		...
		if event.index == _touch_index:
			if not _is_point_inside_drag_area(event.position):
				_reset()
				if visibility_mode == Visibility_mode.WHEN_TOUCHED:
					_base.hide()
				get_viewport().set_input_as_handled()
...
func _is_point_inside_drag_area(point: Vector2) -> bool:
	return _allowed_rect.has_point(point)
